...
- Images and links in full-text (reader-view) articles now resolve correctly when the article page redirects (for example, via a link-tracking hop or a moved site): relative URLs are resolved against the page's final address instead of the original link, so they no longer break. Feed auto-discovery resolves relative feed links the same way.
