...
| Rename feed / custom title | §9.1 | `feeds.user_title` override column; `FeedService.Rename`; `POST /feeds/{id}/rename`; manage-page edit control (mirror category rename) | Must be a **separate** override column: poll does `Title = orKeep(pf.Title, Title)`, so writing a custom name into `feeds.title` gets clobbered on the next poll. `feeds.title` = best automatic name (feed `<title>`, else feed-URL fallback); display = `user_title ?? title` | deferred |
