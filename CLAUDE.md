...
- **Host guard (B11, DNS-rebinding).** `web.New` takes a final `expectedHost` (BaseURL host; tests pass `""` to disable); `hostGuard` `421`s any request whose `Host` ≠ it, compared via `normalizeAuthority` (lowercase + drop default `:80`/`:443`), exempting **only the `/healthz` path** (not loopback Hosts — that would let a same-machine page drive mutating POSTs). `config.Load` rejects a `BFEED_BASE_URL` that isn't an absolute http(s) URL with a host, so the guard can't fail open.
